...
datapre <- subset(datapre, subset = nFeature_RNA > 250 & nFeature_RNA < 5000 & percent.mt < 25 & nCount_RNA <32000)
...
VlnPlot(datapre, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) # Me salen los clusters 2 y 3 (y 9?) raros en features y counts.
DimPlot(datapre,
        reduction = "umap",
        label = TRUE,
        label.size = 6)
...
De momento me han gustado res = 0.2
...
datapre <- AddMetaData(object = datapre, metadata = factor(celltypes), col.name = "projections")
 ...
 datapre@meta.data$newcluster <- listcluster[["0.2"]] #0.4, 0.2
