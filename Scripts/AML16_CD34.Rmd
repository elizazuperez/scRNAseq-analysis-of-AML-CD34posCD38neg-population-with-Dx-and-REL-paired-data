...
plot2 +  geom_hline(yintercept=c(2000,5500)) + geom_vline(xintercept = c(2000,20000))
plot1  + geom_vline(xintercept = c(20000,2000)) + geom_hline(yintercept = 12)
datapre <- subset(datapre, subset = nFeature_RNA >2000 & nFeature_RNA < 5500 & percent.mt < 20 & nCount_RNA <20000 & nCount_RNA > 2000) # He cambiado el porcentaje máximo de genes mitocondriales
...
DimHeatmap(datapre, dims = 1:14, cells = 500, balanced = TRUE) # muchos genes ribosomicos (RPL..., RPS...)
print(x = datapre[["pca"]], 
      dims = 1:13, 
      nfeatures = 5)
...
datapre <- AddMetaData(object = datapre, metadata = factor(celltypes), col.name = "projections")
 ...
 datapre@meta.data$newcluster <- listcluster[["0.5"]] #2
