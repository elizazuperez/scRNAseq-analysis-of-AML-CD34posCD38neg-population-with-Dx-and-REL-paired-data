...
plot2 +  geom_hline(yintercept=c(450,5300)) + geom_vline(xintercept = 38000)
plot1  + geom_vline(xintercept = 38000)
datapre <- subset(datapre, subset = nFeature_RNA > 450 & nFeature_RNA < 5300 & percent.mt < 20 & nCount_RNA <38000)
...
DimHeatmap(datapre, dims = 1:15, cells = 500, balanced = TRUE) 
print(x = datapre[["pca"]], 
      dims = 1:15, 
      nfeatures = 5)
...
res = c(0.2,0.4,0.5,0.6,0.8,1,1.2,1.4,1.6)
 ...
 ## Selected
 datapre@meta.data$newcluster <- listcluster[["0.2"]]
...
# SIGO CON RES =1
plotlist
# Idents(object = datapre) <- "SCT_snn_res.0.8"
VlnPlot(datapre, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
DimPlot(datapre,
        reduction = "umap",
        label = TRUE,
        label.size = 6) #El 8 es raro
...
datapre <- AddMetaData(object = datapre, metadata = factor(celltypes), col.name = "projections")
 ...
 datapre@meta.data$newcluster <- listcluster[["0.6"]] # maybe 0.8
