...
plot2 +  geom_hline(yintercept=c(1000,5500)) + geom_vline(xintercept = c(1500,35000))
plot1  + geom_vline(xintercept = c(1500,35000)) + geom_hline(yintercept = 20)
datapre <- subset(datapre, subset = nFeature_RNA > 1500 & nFeature_RNA < 5500 & percent.mt < 20 & nCount_RNA <35000 & nCount_RNA > 1500)
...
ElbowPlot(datapre, ndims = 40) # Me he quedado con los 14 primeros
DimHeatmap(datapre, dims = 1:22, cells = 500, balanced = TRUE) 
print(x = datapre[["pca"]], 
      dims = 1:22, 
      nfeatures = 5)
...
# Idents(object = datapre) <- "SCT_snn_res.1.4"
 datapre@meta.data$newcluster <- listcluster[["0.7"]]
...
datapre <- AddMetaData(object = datapre, metadata = factor(celltypes), col.name = "projections")
 ...
 datapre@meta.data$newcluster <- listcluster[["0.6"]] #0.7
