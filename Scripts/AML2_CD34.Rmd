...
```
He cambiado el nombre de la carpeta de la muestra en la carpeta que me pasaste.
```{r}
sample <- "AML2_CD34pos"
...
plot2 +  geom_hline(yintercept=c(250,5000))
plot1
datapre <- subset(datapre, subset = nFeature_RNA > 250 & nFeature_RNA < 5000 & percent.mt < 20 & nCount_RNA <42000)
...
DimHeatmap(datapre, dims = 1:20, cells = 500, balanced = TRUE) 
print(x = datapre[["pca"]], 
      dims = 1:20, 
      nfeatures = 5)

dims = 13 # Numero de PCs
```
...
#library(openxlsx)
library(stringr)
velten = openxlsx::read.xlsx("Velten_projections.xlsx")
...
cellcolors <- openxlsx::read.xlsx("veltencolors.xlsx")
...
datapre <- AddMetaData(object = datapre, metadata = factor(celltypes), col.name = "projections")
 ...
 datapre@meta.data$newcluster <- listcluster[["0.4"]] #2

 Idents(object = datapre) <- "newcluster"



plot1 <- DimPlot(datapre, reduction = "umap",  label = TRUE, group.by = "projections", label.size = 2, pt.size = 1.1, cols = colors) + theme(legend.text = element_text(size = 5),
...
df[df== 0] <- NA
View(df)# proporción de celltype per cluster y número total de células anotadas con la proyección en el cluster
```
...
openxlsx::write.xlsx(dataexport, paste0(results,"metadata.xlsx"))
...
openxlsx::write.xlsx(df, paste0(results,"projections.xlsx"))
openxlsx::write.xlsx(data.markers, paste0(results,"Markers.xlsx"))
...
openxlsx::write.xlsx(enrichlist, paste0(results,"enrichment.xlsx"))
