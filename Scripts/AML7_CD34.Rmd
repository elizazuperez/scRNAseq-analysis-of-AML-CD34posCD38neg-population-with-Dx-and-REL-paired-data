...
plot2 +  geom_hline(yintercept=c(600,4200)) + geom_vline(xintercept = 26000)
plot1  + geom_vline(xintercept = 30000)
datapre <- subset(datapre, subset = nFeature_RNA > 600 & nFeature_RNA < 4200 & percent.mt < 20 & nCount_RNA <26000)
...
ElbowPlot(datapre, ndims = 40) # Me he quedado con los 15 primeros
DimHeatmap(datapre, dims = 1:12, cells = 500, balanced = TRUE) 
print(x = datapre[["pca"]], 
      dims = 1:12, 
      nfeatures = 5)
...
myClusterFUN <- function(x, res) {
  g <- bluster::makeSNNGraph(x,
                             ...
                             k = 20,
                             BNPARAM = AnnoyParam())
  igraph::cluster_louvain(g, resolution = res)$membership 
}


res = c(0.2,0.4,0.5,0.6,0.8,1,1.2,1.4,1.6)
 ...
 ## Selected
 datapre@meta.data$newcluster <- listcluster[["0.5"]]
...
DimPlot(datapre,
        reduction = "umap",
        label = TRUE,
        label.size = 6) #El 9 es raro
# Me quedo con resolution = 
...
datapre <- AddMetaData(object = datapre, metadata = factor(celltypes), col.name = "projections")
 ...
 datapre@meta.data$newcluster <- listcluster[["0.5"]] #1.4
...
sample <- "AML7_CD34pos"
aml7dir <- "./results/separated/AML7_CD34pos/" 
aml7 <- readRDS(paste0(aml7dir, sample,".rds"))
...
cellcolors <- read.xlsx("veltencolors.xlsx")
rownames(cellcolors) = cellcolors[,1]
cellcolors <- cellcolors[levels(aml7@meta.data$projections),]

colors <- cellcolors$cols

pdf(paste0(aml7dir, "finalDimplot_Clusters.pdf"))
DimPlot(aml7)
dev.off()

pdf(paste0(aml7dir, "finalDimplot_Projections.pdf"))
DimPlot(aml7, group.by = "projections", cols = colors) + theme(legend.position = "none") + ggtitle("")
dev.off()
