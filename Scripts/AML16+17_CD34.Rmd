...
```{r}
Dx <- readRDS("./results/separated/AML16_CD34pos/AML16_CD34pos.rds")
Dx@meta.data$orig.ident <- rep("Dx",nrow(Dx@meta.data))
REL <- readRDS("./results/separated/AML17_CD34pos/AML17_CD34pos.rds")
...
plotlist
 ...
 ## Selected
 aml.combined@meta.data$newcluster <- listcluster[["0.7"]]
...
```{r}
library(kableExtra)
exportdir <- "./results/integrated/AML16.17_CD34/"
...
filtered_data <- REL@assays$SCT@scale.data[which(rownames(REL@assays$SCT@scale.data)%in%unlist(LSC_signature)),]
...
library(RColorBrewer)
FeaturePlot(aml.combined,features = "CD44", pt.size = 1.3, cols = rev(brewer.pal(n = 11, name = "RdBu")), shape.by = "orig.ident") + 
  DimPlot(aml.combined, reduction = "umap", pt.size = 1)
```
# Export
```{r}
exportdir <- "./results/integrated/AML16.17_CD34/"
dir.create("./results/integrated")
dir.create(exportdir)
saveRDS(aml.combined, paste0(exportdir, "AML16.17_integrated.rds"))
