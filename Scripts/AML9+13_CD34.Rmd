...
```{r}
Dx <- readRDS("./results/separated/AML9_CD34pos/AML9_CD34pos.rds")
Dx@meta.data$orig.ident <- rep("Dx",nrow(Dx@meta.data))
REL <- readRDS("./results/separated/AML13_CD34pos/AML13_CD34pos.rds")
...
plotlist
 ...
 ## Selected
 aml.combined@meta.data$newcluster <- listcluster[["0.5"]]
...
DimPlot(aml.combined, reduction = "umap", group.by = "orig.ident", pt.size = 1.5)

DimPlot(aml.combined, reduction = "umap", split.by = "orig.ident", pt.size = 1.5)
aml.combined$orig.combined <- paste(aml.combined$orig.ident, aml.combined$orig.cluster, sep = ".")
DimPlot(aml.combined, reduction = "umap", group.by =  "orig.combined", pt.size = 1.5, label = T)


aml.combined$orig.ident <- factor(aml.combined$orig.ident)
 DimPlot(aml.combined, reduction = "umap",  label = F,
                 group.by = "projections", label.size = 2, pt.size = 1.5, cols = colors, split.by = "orig.ident") +
...
# Export
```{r}
exportdir <- "./results/integrated/AML9.13_CD34/"
dir.create("./results/integrated")
dir.create(exportdir)
saveRDS(aml.combined, paste0(exportdir, "AML9.13_integrated.rds"))
